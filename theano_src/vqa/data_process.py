...
def load_data(args):
    dep = False
    args.train_q = args.train_path+'binary_questions.txt'
    args.train_a = args.train_path+'binary_ans.txt'
    args.train_id = args.train_path+'binary_id.txt'
    args.train_obj = args.train_path+'object.json'
    args.val_q = args.valid_path+'binary_questions.txt'
    args.val_a = args.valid_path+'binary_ans.txt'
   ...
   # get word dict
    ...
    words2idx = {PADDING: 0, OOV: 1}

    for w in words:
        if w not in words2idx:
            words2idx[w] = len(words2idx) 
    print 'voc_size: %s'%len(words2idx)

    objs2idx = {PADDING: 0, OOV: 1}
    objs = [w for sub_corpus, _ in corpus for sent in sub_corpus[2] for w in sent ]
    for w in objs:
        if w not in objs2idx:
            objs2idx[w] = len(objs2idx) 
    print 'obj_size: %s'%len(objs2idx)
    if dep:
        print 'arc_type_dict:', len(arc_type_dict), arc_type_dict
    train_set_q, train_set_a, train_set_obj = [], [], [] 
    valid_set_q, valid_set_a, valid_set_obj = [], [], []
    if dep:
        train_set_dep = []
        valid_set_dep = []
        test_set_dep = []
    ...
    q, a, obj = corp
    arc_type_dict = None
    collect_data(train_set_q, train_set_a, train_set_obj, train_set_dep, q, a, obj, dependencies, words2idx, objs2idx, arc_type_dict, dep)
    corp, dependencies = dev_corpus

    q, a, obj = corp
    collect_data(valid_set_q, valid_set_a, valid_set_obj, valid_set_dep, q, a, obj, dependencies, words2idx, objs2idx, arc_type_dict, dep)
    train = [train_set_q, train_set_a, train_set_obj]
    valid = [valid_set_q, valid_set_a, valid_set_obj]
...
def collect_data(corpus_q, corpus_a, corpus_obj, corpus_dep, q, a, obj, dependencies, words2idx, objs2idx, arc_type_dict, dep):
    corpus_q.extend( [[words2idx.get(w, 0) for w in sent]  for sent in q] )
    corpus_a.extend(a)
    corpus_obj.extend( [[objs2idx.get(w, 0) for w in sent] for sent in obj])
    """
    if dep:
        if add:
...
def prepare_data(seqs, objs, x_mask_ori=None, obj_mask_ori=None, maxlen=None):
    ...
    for idx, s in enumerate(seqs):
        x[:lengths[idx], idx, :] = s
        if x_mask_ori is not None:
            x_mask[:lengths[idx], idx, :lengths[idx], :] = x_mask_ori[idx][:lengths[idx], :lengths[idx], :]
    ...
    # x: a list of sentences
    lengths = [len(s) for s in objs]

    # This part: suspeciously wrong.
    if maxlen is not None:
        new_objs = []
        new_lengths = []
        for l, s in zip(lengths, objs):
            if l < maxlen:
                new_objs.append(s)
                new_lengths.append(l)
            else:
                new_objs.append(s[:maxlen])
                new_lengths.append(maxlen)
        lengths = new_lengths
        objs = new_objs
        if len(lengths) < 1:
    ...
    n_samples = len(objs)
    maxlen = numpy.max(lengths)
    #print len(objs), n_samples 
    #print 'object shape:', objs[0].shape
    assert objs[0].ndim == 2
    obj = numpy.zeros((maxlen, n_samples, objs[0].shape[1])).astype('int32')
    if obj_mask_ori is not None:
        obj_mask = numpy.zeros((maxlen, n_samples, maxlen, mask[0].shape[-1])).astype(theano.config.floatX)
    else:
        obj_mask = numpy.zeros((maxlen, n_samples)).astype(theano.config.floatX)
    
    for idx, s in enumerate(objs):        
        obj[:lengths[idx], idx, :] = s        
        if obj_mask_ori is not None:
            obj_mask[:lengths[idx], idx, :lengths[idx], :] = obj_mask_ori[idx][:lengths[idx], :lengths[idx], :]
            #if maxlen > lengths[idx]:
            #    x_mask[lengths[idx]:, idx, lengths[idx]:, 0] = numpy.eye(maxlen-lengths[idx], dtype=theano.config.floatX)
            '''for i in range(len(mask[idx])):
                print 'batch', idx, 'position', i, 'length:', lengths[idx]
            ...
            obj_mask[:lengths[idx], idx] = 1.




    #print 'x shape:', x.shape, 'obj shape:', obj.shape
    #print "x_mask",x_mask
    #print "obj_mask",obj_mask
    return x, x_mask, obj, obj_mask
