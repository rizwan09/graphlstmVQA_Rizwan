...
def gen_miwa_files_from_json(infile, out_dir):
    import json
            ...
            for item in content:
                ...
                for sentence in sentences:
                    ...
                    try:
                        ...
                        root = 0
                        sys.stderr.write('Sentence NO ROOT! ===== Instance information: filename: ' + str(count) + '.txt, PMID: '+str(item['article']) + 
                                ', Paragraph '+str(sentence['paragraph'])+', sentence '+str(sentence['sentence'])+'\n')
                    for node in sentence['nodes']:
                        origin_text.append(remove_non_ascii(re.sub('[ \t]', '', node['label'].strip())))
                        lemma.append(remove_non_ascii(re.sub('[ \t]', '', node['lemma'].strip())))
                        pos_tags.append(node['postag'])
                        arc_list = node['arcs']
                        node_idxs.append(node['index']) #-prev_sent_length
                        arcs = []
                        local_arc_dict = {}
                        if int(node['index']) == int(root):
                            if pre_root == -1:
                                arc_info = ('ROOT', -1)
                            else:
                                arc_info = ('PREVROOT', pre_root)
                            #sys.stderr.write('root!!!!' + str(arc_info) +'\n')
                        else:
                          for arc in arc_list:
                            alabels = arc['label'].split(':')
                            if alabels[0] in local_arc_dict:
                                continue
                            if alabels[0] == dep_label or (alabels[0] == adj_label and alabels[1] == 'prev'):
                                try:
                                    assert int(arc['toIndex']) != int(node['index'])
                                    local_arc_dict[alabels[0]] = (alabels[1].split('_')[0], arc['toIndex'])
                                except:
                                    local_arc_dict[alabels[0]] = ('toRoot', root)
                                    #sys.stderr.write(str(node['index']) + '\t' + str(root)+'\n')
                                    sys.stderr.write('arc point to self!!!! Node:'+str(node['index'])+','+node['label']+'\t'+str(arc_list)+'. Printing out the \n')
                                    sys.stderr.write('===== Instance information: filename: ' + str(count) + '.txt, PMID: '+str(item['article']) + 
                                        ', Paragraph '+str(sentence['paragraph'])+', sentence '+str(sentence['sentence'])+', paraSent:'+str(sentence['paragraphSentence'])+'\n')
                                    sys.stderr.write('Current local_arc_dict: ' + str(local_arc_dict)+'\n')
                                    for node in sentence['nodes']:
                                        sys.stderr.write(node['label']+'\t'+str(node['arcs'])+'\n')
                          try:
                            assert len(local_arc_dict) != 0 or node['index'] == root
                          except:
                            sys.stderr.write('Node '+str(node['index'])+' does not have dependency! ===== Instance information: filename: ' + str(count) + '.txt, PMID: '+str(item['article']) + 
                                ', Paragraph '+str(sentence['paragraph'])+', sentence '+str(sentence['sentence'])+'\n')
                            local_arc_dict[dep_label] = ('toRoot', root)
                          if dep_label in local_arc_dict:
                            arc_info = local_arc_dict[dep_label]
                          else:
                            arc_info = local_arc_dict[adj_label]
                        sys.stderr.write('the final local_arc_dict: ' + str(local_arc_dict) + '\n')
                        sys.stderr.write('the final add arc: ' + str(arc_info) + '\n')
                        arc_labels.append(re.sub('[\(\)]', '', arc_info[0]))
                        arc_to_idxs.append(arc_info[1])
                    pre_root = root
                indices = []
                assert len(origin_text) == len(lemma)
                assert len(origin_text) == len(pos_tags)
                assert len(origin_text) == len(node_idxs)
                assert len(origin_text) == len(arc_labels)
                assert len(origin_text) == len(arc_to_idxs)
                for entity in entities:
                    ...
                    for eidx in range(start, end):
                        ...
                        lemma[eidx] = '<ANNO_TYPE_'+entity['type']+'>'
                #if relation != 'None':
                #    print count, len(origin_text)
                with cs.open(os.path.join(out_dir, str(count)+'.txt.conll'), 'w', encoding='utf-8') as coutf, \
                        cs.open(os.path.join(out_dir, str(count)+'.txt'), 'w', encoding='utf-8') as toutf, \
                        cs.open(os.path.join(out_dir, str(count)+'.ann'), 'w', encoding='utf-8') as aoutf:
                    for idx, wd, lem, pos, atidx, al in zip(node_idxs, origin_text, lemma, pos_tags, arc_to_idxs, arc_labels):
                        coutf.write('\t'.join([str(idx+1), wd, lem, pos, '_', str(atidx+1), al]) + '\n')
                    #toutf.write(re.sub('\t', ' ', ' '.join(origin_text))+'\n')
                    toutf.write(' '.join(origin_text)+'\n')
                    for ec, idx in enumerate(indices):
                        aoutf.write('T'+str(ec+1)+'\tTerm '+str(len(' '.join(origin_text[:idx[0]]))+1) + ' ' +str(len(' '.join(origin_text[:idx[-1]+1]))) + '\t' +' '.join(origin_text[idx[0]:idx[-1]+1])+'\n')
                    if relation != 'None':
                        relation = 'Pos'
                    aoutf.write('R'+str(count)+'\t'+relation+' Arg1:T1 Arg2:T2\n')
                count += 1
...
def prepare_data(seqs, eidxs, mask=None, maxlen=None):
    ...
    lengths = [len(s) for s in seqs[0]]

    # This part: suspeciously wrong.
    if maxlen is not None:
        new_seqs = []
        new_lengths = []
        for lseq in zip(lengths, *seqs):
            l = lseq[0]
            s = lseq[1:]
            if l < maxlen:
                new_seqs.append(s)
                ...
                new_seqs.append([el[:maxlen] for el in s])
    ...
    n_samples = len(seqs[0])
    maxlen = numpy.max(lengths)
    #print len(seqs), n_samples 
    #print 'sequence shape:', seqs[0][0].shape
    assert seqs[0][0].ndim == 2
    xs = [numpy.zeros((maxlen, n_samples, seq[0].shape[1])).astype('int32') for seq in seqs]
    if mask is not None:
    ...
    for idx, ss in enumerate(zip(*seqs)):
        for i, s in enumerate(ss):
            xs[i][:lengths[idx], idx, :] = s
        if mask is not None:
...
# give the original input, convert them to idxs
...
# give the original input, convert them to idxs
def collect_data_multi_input(corpus_x, corpus_y, corpus_idx, corpus_dep, x, y, idx, dependencies, anno2idxs, arc_type_dict, dep, add):
    assert len(x[0][0]) == len(anno2idxs)
    for i in range(len(anno2idxs)):
        corpus_x[i].extend( [[anno2idxs[i].get(w[i], 0) for w in sent]  for sent in x] )
    corpus_y.extend(y)
    corpus_idx.extend(idx)
    if dep:
        if add:
        ...
        corpus_dep.extend(child_exists)
        assert len(corpus_idx) == len(corpus_dep)
    assert len(corpus_x[0]) == len(corpus_y)
    assert len(corpus_y) == len(corpus_idx)
...
def read_file_multi_class(filename, label_dict, num_entities=2, labeled=True):
    corpus_x = []
    corpus_y = []
    corpus_idx = []
#        urlStr = 'http[s]?://(?:[a-zA-Z]|[1-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
...
def load_data(train_path=None, valid_path=None, test_path=None, num_entities=2, dep=False, train_dep=None, valid_dep=None, test_dep=None, add=True):
    ...
    train_corpus = read_file_multi_class(train_path, label_dict, num_entities)
    if dep:
    ...
    # load dev data
    dev_corpus = read_file_multi_class(valid_path, label_dict, num_entities)
    if dep:
    ...
    if dep:
        ...
        if test_path is not None:
            test.append(None)
    #labels2idx = {'+':1, '-':0}
    dics = {'words2idx': words2idx, 'labels2idx': label_dict} 
    if dep:
        dics['arcs2idx'] = arc_type_dict
    if test_path is not None:
        return [train, valid, test, dics]
...
def read_file_multi_input_multi_class(filename, label_dict, num_entities=2, labeled=True):
    corpus_x = []
    corpus_y = []
    corpus_idx = []
#        urlStr = 'http[s]?://(?:[a-zA-Z]|[1-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
    with cs.open(filename, 'r', encoding='utf-8') as inf:
        ...
        for line in inf:
            ...
            assert len(entity_id_arry) == num_entities
            assert len(elems) == num_entities + 2
            # Here is the major difference: we assume x contains several information: token, pos, ner, etc. They are splitted by '/'
            xs = [tks.split('|||') for tks in elems[0].lower().split(' ')]
            label = elems[-1]
            try:
                for i in range(num_entities):
                    assert entity_id_arry[i][-1] < len(xs)
            except:
                sys.stderr.write('abnormal entity ids:'+str(entity_id_arry)+', sentence length:'+str(len(xs))+'\n')
                continue
            #sentence = stringQ2B(sentence)
            if len(xs) < 1:
                print xs 
...
def load_data_withAnnos(train_path=None, valid_path=None, test_path=None, num_entities=2, dep=False, train_dep=None, valid_dep=None, test_dep=None, add=True):
    print 'loading training data from', train_path, 'loading valid data from', valid_path, 'loading test data from', test_path
    corpus = []
    arc_type_dict = dict()
    label_dict = {'None': 0}
    # load training data
    #train_corpus = read_file(os.path.join(train_path, content_fname+'.train'), num_entities)
    train_corpus = read_file_multi_input_multi_class(train_path, label_dict, num_entities)
    if dep:
        assert train_dep is not None
        dependencies, arc_type_dict = read_graph_dependencies(train_dep, arc_type_dict, add)
        corpus.append((train_corpus, dependencies))
        ...
        corpus.append((train_corpus, None))
    # load dev data
    dev_corpus = read_file_multi_input_multi_class(valid_path, label_dict, num_entities)
    if dep:
        assert valid_dep is not None
        dependencies, arc_type_dict = read_graph_dependencies(valid_dep, arc_type_dict, add)
        corpus.append((dev_corpus, dependencies))
        ...
        corpus.append((dev_corpus, None))
    # get word dict
    #print len(train_corpus[0][0][0]) 
    #assert len(train_corpus[0][0][0]) == 2
    annos = [[] for el in train_corpus[0][0][0]]
    for i in range(len(train_corpus[0][0][0])):
        annos[i] = [elem[i] for sub_corpus, _ in corpus for sent in sub_corpus[0] for elem in sent ]
    # Special treatment for the final PubMed experiments
    #words = [w for sent in train_corpus[0] for w in sent ]
    print 'number of annos:', len(annos)
    words2idx = {OOV: 0}
    anno2idxs = [words2idx] + [dict() for i in annos[1:]]
    for ann, dic in zip(annos, anno2idxs):
        for a in ann:
            if a not in dic:
                dic[a] = len(dic)    
        print 'voc_size:', len(dic)
    #print 'print pos dict:'
    #for elem in anno2idxs[-1]:
    #    print elem, anno2idxs[-1][elem]
    if dep:
        print 'arc_type_dict:', len(arc_type_dict), arc_type_dict
    train_set_x, train_set_y, train_set_idx = [[] for el in anno2idxs], [], [] 
    valid_set_x, valid_set_y, valid_set_idx = [[] for el in anno2idxs], [], []
    test_set_x, test_set_y, test_set_idx = [[] for el in anno2idxs], [], []
    ...
    print 'check entity of training data'
    check_entity(x, idx)
    # Note, the shape of x = [[[num_annos] num_words_in_sent] num_sent]
    collect_data_multi_input(train_set_x, train_set_y, train_set_idx, train_set_dep, x, y, idx, dependencies, anno2idxs, arc_type_dict, dep, add)
    corp, dependencies = dev_corpus
    x, y, idx = corp
    print 'check entity of dev data'
    check_entity(x, idx)
    collect_data_multi_input(valid_set_x, valid_set_y, valid_set_idx, valid_set_dep, x, y, idx, dependencies, anno2idxs, arc_type_dict, dep, add)
    train = [train_set_x, train_set_y, train_set_idx]
    valid = [valid_set_x, valid_set_y, valid_set_idx]
    if test_path is not None:
        test = [test_set_x, test_set_y, test_set_idx]
    if dep:
        ...
        if test_path is not None:
            test.append(None)
    #labels2idx = {'+':1, '-':0}
    dics = {'words2idx': words2idx, 'labels2idx': label_dict, 'poss2idx': anno2idxs[-1]} 
    if dep:
        dics['arcs2idx'] = arc_type_dict
    if test_path is not None:
        return [train, valid, test, dics]
