...
def conv_data(corpus, win_l, win_r):
    lexv = []
    objv = []
    temp_lex, temp_y, temp_obj, temp_dep = corpus
    for i, (x,y,obj) in enumerate(zip(temp_lex, temp_y, temp_obj)):
        words = conv_x(x, win_l, win_r)
        objs = conv_x(obj, win_l, win_r)
        assert len(words) == len(x)
        assert len(objs) == len(obj)
        objv.append(objs)
        lexv.append(words)
    assert len(lexv) == len(objv)
    assert len(lexv) == len(temp_y)
    return [lexv, temp_y, objv, temp_dep]

def conv_data_graph(corpus, win_l, win_r, multi_input=False):
    lexv = []
#    labv = []
    idxv = []
    temp_lex, temp_y, temp_idx, temp_dep = corpus
    #print 'temp_lex:', len(temp_lex), len(temp_lex[0]), len(temp_lex[0][0])
    if multi_input:
        lexv = [[] for el in temp_lex]
        convert = temp_lex + [temp_idx ]
    else:
        convert = [temp_lex, temp_idx]
    #print 'input size:', len(convert)
    for elems in zip(*convert):
        #print 'elems size:', len(elems)
        if multi_input:
            for anno, x in zip(lexv,elems[:-1]):
                words = conv_x(x, win_l, win_r)
                assert len(words) == len(x)
                anno.append(words)
        else:
            words = conv_x(elems[0], win_l, win_r)
            assert len(words) == len(elems[0])
            lexv.append(words)
        npidx = []
        for ii in elems[-1]:
	...
	idxv.append(npidx)
    if multi_input:
        assert len(lexv[0]) == len(idxv)
    else:
        assert len(lexv) == len(idxv)
    #print len(idxv)
    #print len(temp_y)
    #assert len(idxv) == len(temp_y)
...
#def sgd(lr, tparams, grads, cost, att_wts, etv, prefix, *input_params): #x_f, x_w, y, 
    ...
    f_cost = theano.function(inputs=list(input_params),
            outputs=cost, #att_wts, etv],
...
def create_relation_circuit(_args, StackConfig):
    ...
    if _args.graph or _args.batch_size > 1:
        ...
        x_w, o_w,  mask_x, mask_o, y, y_pred, cost, grads, regularizable_params = _args.circuit(cargs)
        inputs = [x_w, o_w, y, mask_x, mask_o]
        pred_inputs = [x_w, o_w, mask_x, mask_o]
    else:
        raise Exception("No Implementaiton")
        #print _args.circuit(cargs)
        #x_ws, o_ws, y, y_pred, cost, grads, regularizable_params = _args.circuit(cargs)
        #inputs = [x_ws, o_ws, y]
        #pred_inputs = [x_ws, o_ws]
    if _args.verbose == 2:
        print "\n", "Printing Configuration after StackConfig:"
        print_args(cargs)
    lr = T.scalar('lr')
...
def create_multi_task_circuit(_args, StackConfig, num_tasks):
    cargs = dict_from_argparse(_args)
    cargs = StackConfig(cargs)
    for i in range(num_tasks):
        cargs['d'+str(i)+'_hidden_output_trans_out_dim'] = len(cargs['idx2label_dicts'][i])
    f_arr, x_arr, y_arr, pred_y_arr, costs_arr, cost, grads_arr, grads, regularizable_param_arr, global_regularizable_params = _args.circuit(cargs, num_tasks)
    return build_obj_grad_and_classifier(_args, cargs, f_arr, x_arr, y_arr, pred_y_arr, costs_arr, grads_arr, regularizable_param_arr, cost, grads, global_regularizable_params)

def create_multi_domain_multi_task_circuit(_args, StackConfig, num_domains, num_tasks):
    cargs = dict_from_argparse(_args)
    cargs = StackConfig(cargs)
    cargs['m1_wemb1_shared_dim'] = cargs['m1_wemb1_out_dim']/2
    cargs['m1_lstm_shared_dim'] = cargs['m1_lstm_out_dim']/(1+num_domains)
    for i in range(num_domains):
        cargs['m1_wemb1_domain_'+str(i)+'_dim'] = cargs['m1_wemb1_out_dim']/(2*num_domains)
        cargs['m1_lstm_domain_'+str(i)+'_dim'] = cargs['m1_lstm_out_dim']/(1+num_domains)
    for i in range(num_tasks):
        cargs['d'+str(i)+'_hidden_output_trans_out_dim'] = len(cargs['idx2label_dicts'][i])
    f_arr, x_arr, y_arr, pred_y_arr, costs_arr, cost, grads_arr, grads, regularizable_param_arr, global_regularizable_params = _args.circuit(cargs, num_domains, num_tasks)
    print [p.name for p in global_regularizable_params]
    return build_obj_grad_and_classifier(_args, cargs, f_arr, x_arr, y_arr, pred_y_arr, costs_arr, grads_arr, regularizable_param_arr, cost, grads, global_regularizable_params)

def build_obj_grad_and_classifier(_args, cargs, f_arr, x_arr, y_arr, pred_y_arr, costs_arr, grads_arr, regularizable_param_arr, cost=None, grads=None, global_regularizable_params=None):
    if _args.verbose == 2:
        print "\n", "Printing Configuration after StackConfig:"
        print_args(cargs)
    lr = T.scalar('lr')
