...
class Chip(object):
    ...
    def _declare_mat(self, name, *dim, **kwargs):
        ...
        # by default, it is a parameter and can be trained
        var.is_trainable = True
...
class Start(object):
    """ A start object which basically take the input 
    and start off the architecture building process.
    In a generalized version, the output_tv can be an array.
    """
...
# take several inputs and convert them to embeddings and concatenate to an output.
class Multi_Embeddings(Chip):
    def prepend(self, previous_chip):
        self = super(Multi_Embeddings, self).prepend(previous_chip)
        # Note: here the wemb_matrix is an array of pre_trained embedding matrics.
        # Do not support on-the-fly definition anymore
        if 'emb_matrices' in self.params:
            print 'pre_trained embedding!!'
            self.T_ = self.params['emb_matrices']
    ...
    def compute(self, inputs):
        ...
        print 'in multi_embeddings', len(inputs), len(self.T_)
        def compute_one_input(T_, in_tv, out_dim):
            print in_tv, type(in_tv)
            n_timesteps = in_tv.shape[0]
            window_size = 1
            if in_tv.ndim == 2:
                window_size = in_tv.shape[1] 
            elif in_tv.ndim == 3:
                batch_size = in_tv.shape[1]
                window_size = in_tv.shape[2]
            print 'in_tv dimension:', in_tv.ndim
            print 'window size = ', window_size
            if in_tv.ndim < 3:
                output_tv = T_[in_tv.flatten()].reshape([n_timesteps, window_size * out_dim], ndim=2)
            else:
                output_tv = T_[in_tv.flatten()].reshape([n_timesteps, batch_size, window_size * out_dim], ndim=3)
            return output_tv
        
        outputs = []
        for i, in_tv in enumerate(inputs):
            outputs.append(compute_one_input(self.T_[i], in_tv, self.T_[i].shape[1]))
        self.output_tv = T.concatenate(outputs, axis=-1)
...
class Embedding(Chip):
    def prepend(self, previous_chip):
        ...
        if 'qemb_matrix' in self.params and 'question' in  self.name:
            print 'pre_trained embedding!!'
            self.T_ = self.params['qemb_matrix']
            print self.T_, type(self.T_)
        elif 'oemb_matrix' in self.params and 'object' in  self.name:
            print 'pre_trained embedding!!'
            self.T_ = self.params['oemb_matrix']
            print self.T_, type(self.T_)
        else:
            self.T_ = self._declare_mat('T', self.params['o_voc_size'], self.out_dim)
    ...
    def compute(self, input_tv):
        ...
        if self.params.get(self.kn('dropout_rate'), 0.0) != 0.0:
            print 'DROP OUT!!! at circuite', self.name, 'Drop out rate: ', self.params[self.kn('dropout_rate')]
            self.output_tv = _dropout_from_layer(self.params['rng'], self.output_tv, self.params[self.kn('dropout_rate')])
        # Note: when we import the pre-defined wemb_matrix, we do not add it to internal parameters because we already defined it as a regularizable parameter.
        #if 'wemb_matrix' in self.params:
        #    return tuple() 
        #else:
...
# compute attention according to the similarity between questions and objects
class Question_Object_attention(Chip):
    def prepend(self, question_chip, object_chip):
        self.W = self._declare_mat('W', question_chip.out_dim, object_chip.out_dim)
    ...
    def get_att_weights(self, question_tv, object_tv):
        # temp_tv dimension: (sent_q_len, batch_size, otv_dim)
        temp_tv = T.dot(question_tv, self.W)
        # object_tv dimension: (sent_o_len, batch_size, otv_dim)
        # get a new tensor of dim: (sent_q_len, sent_o_len, batch_size)
        return T.sum(temp_tv[:, None, :, :] * object_tv[None, :, :, :], axis=3)
        #return T.sum(question_tv[:,None,:,:]*object_tv[None,:,:,:], axis=3)

    def compute(self, question_tv, object_tv, mask_q=None, mask_o=None):
        attention_tv = self.get_att_weights(question_tv, object_tv)
        #temp_tv dimension: (sent_q_len, sent_o_len, batch_size, qtv_dim+otv_dim)

        #temp2_tv dim = temp_tv dim
        temp_qtv = question_tv[:,None,:,:]*T.ones([object_tv.shape[0]])[None, :, None, None]
        #T.ones([question_tv.shape[0], object_tv.shape[1], question_tv.shape[2], question_tv.shape[3]])

        temp_otv = object_tv[None,:,:,:]*T.ones([question_tv.shape[0]])[:, None, None, None]
        #_like(question_tv)[:, None, :, :
        temp_tv = T.concatenate([temp_qtv, temp_otv],axis=3)
        temp2_tv = attention_tv[:,:,:,None]*temp_tv
        total_cell = T.sum(mask_q, 0)*T.sum(mask_o,0)
        #+T.ones_like(T.sum(mask_q,0))
        self.output_tv = T.sum(temp2_tv,axis=[0,1])/total_cell[:,None]

#class VQA_pooling(Chip):
...
class MeanPooling(Chip):
    def prepend(self, previous_chip):
        self = super(MeanPooling, self).prepend(previous_chip)
        return self
    ...
    """
    def compute(self, input_tv):
        self.output_tv = T.mean(input_tv,axis=0)
...
class LSTMwithAtt(Chip):
    def prepend(self, previous_chip):
        self = super(LSTMwithAtt, self).prepend(previous_chip)
        print 'lstm in dim:', self.in_dim, 'out dim:', self.out_dim
        self.go_backwards = self.params[self.kn('go_backwards')]
        self.win_size = self.params[self.kn('win_size')]
	self.W = self._declare_mat('W', self.in_dim, 4*self.out_dim)
        ...
        self.p = self._declare_mat('p', 3*self.out_dim)
       	self.W_a = self._declare_mat('W', self.out_dim, self.out_dim)
       	self.W_c = self._declare_mat('T', 2*self.out_dim, self.out_dim)
	self.parameters = [self.W, self.U, self.b, self.p, self.W_a, self.W_c]
        return self
    ...
    def compute(self, input_tv, mask=None):
        ...
        def __step_batch(x_, m_, h_prev, c_prev, U, W, W_c, p):
            # Shape: (4*out_dim, num_sample).T + (num_sample, 4*out_dim)
            preact = T.dot(h_prev[:, -1, :], U) + x_
            i = T.nnet.sigmoid(__slice(preact, 0, self.out_dim) + __slice(p, 0, self.out_dim)*c_prev) # Input gate
            f = T.nnet.sigmoid(__slice(preact, 1, self.out_dim) + __slice(p, 1, self.out_dim)*c_prev) # Forget gate
            z = T.tanh(__slice(preact, 3, self.out_dim)) # block input
            c = f * c_prev + i * z # cell state
            ...
            # compute attention
            temp_h = T.dot(W, h.T) # shape: (output_dim, num_sample)
            # shape: (num_sample, win_size)
            att_score = (T.concatenate((h_prev, h[:,None,:]), axis=1) * temp_h.T[:, None, :]).sum(axis=2)
            #att_score = (h_prev * temp_h.T[:, None, :]).sum(axis=2)
            att_weight = T.nnet.softmax(att_score)
            context_h = (att_weight[:, :, None] * T.concatenate((h_prev, h[:,None,:]), axis=1)).sum(axis=1)
            context_h = T.tanh(T.dot(T.concatenate((h, context_h), axis=1), W_c))
            context_h = m_[:, None] * context_h + (1. - m_)[:, None] * h_prev[:, -1, :]
            return T.concatenate((h_prev, context_h[:, None, :]), axis=1)[:, 1:, :], c

        x_in = T.dot(input_tv, self.W).astype(config.floatX) + self.b
        seq_in = [x_in]
        lstm_step = __step
        h_init = T.alloc(np_floatX(0.), self.win_size, self.out_dim)
        c_init = T.alloc(np_floatX(0.), self.out_dim)
        #c_init = T.alloc(np_floatX(0.), self.win_size, self.out_dim)
        if mask is not None:
            seq_in.append(mask)
            lstm_step = __step_batch
            h_init = T.alloc(np_floatX(0.), n_samples, self.win_size, self.out_dim)
            c_init = T.alloc(np_floatX(0.), n_samples, self.out_dim)
            #c_init = T.alloc(np_floatX(0.), n_samples, self.win_size, self.out_dim)
        print 'lstm step:', lstm_step
        rval, _ = theano.scan(lstm_step,
                              sequences=seq_in,
                              outputs_info=[h_init, c_init],
                              non_sequences=[self.U, self.W_a, self.W_c, self.p],
        ...
        if input_tv.ndim == 3:
            self.output_tv = reverse(rval[0])[:,:,-1,:] if self.go_backwards else rval[0][:,:,-1,:]
        else:
            self.output_tv = reverse(rval[0])[:,-1,:] if self.go_backwards else rval[0][:,-1,:]
        if self.params.get(self.kn('dropout_rate'), 0.0) != 0.0:
            print 'DROP OUT!!! at circuite', self.name, 'Drop out rate: ', self.params[self.kn('dropout_rate')]
...
class GraphLSTM(Chip):
    ...
    def compute(self, input_tv, mask=None):
        ...
        def __step_batch(x_, children_mask, t, node_h, node_c, U):
            ...
            node_h = T.set_subtensor(node_h[:, t, :], curr_h)
            node_c = T.set_subtensor(node_c[:, t, :], curr_c)
            #exist_check = T.zeros_like(children_mask)
            #exist_check = T.set_subtensor(exist_check[:, t], 1.)
            #m_ = (children_mask - exist_check).sum(axis=1)
            #node_h = m_[:, None, None]*temp_h + (1 - m_[:, None, None])*node_h
            #node_c = m_[:, None, None]*temp_c + (1 - m_[:, None, None])*node_c
            return node_h, node_c
...
class GraphLSTM_ArcPred(Chip):
    def __init__(self, name, params=None):
        super(GraphLSTM_ArcPred, self).__init__(name, params)
        #self.params[self.name+'_arcType_Emb_hidden_dim'] = self.params[self.kn('type_dim')]
        #self.params[self.name+'_arcType_Emb_out_dim'] = self.params[self.kn('arc_types')]
        #self.arcTypeEncoder = AutoEncoder(name+'_arcType_Emb', self.params)
        self.params[self.name+"_encoder_out_dim"] = params[self.kn('type_dim')]
        self.params[self.name+"_decoder_out_dim"] = params[self.kn('arc_types')]
        self.Encoder = Onehot_Linear(name+'_encoder', self.params)
        self.Decoder = BiasedLinear(name+'_decoder', self.params)
        self.score = 0.
    
    def prepend(self, previous_chip):
        self = super(GraphLSTM_ArcPred, self).prepend(previous_chip)
        print 'graph lstm in dim:', self.in_dim, 'out dim:', self.out_dim
        # Featurize the arc_type auto-encoder
        arc_dim = self.params[self.kn('type_dim')]
        self.Decoder.prepend(self.Encoder.prepend(Start(self.params[self.kn('arc_types')], None)))
        # Featurize the LSTM
        self.go_backwards = self.params[self.kn('go_backwards')]
        self.W = self._declare_mat('W', self.in_dim, 4*self.out_dim)
        temp_U = np.zeros((arc_dim+self.out_dim, 4*self.out_dim)).astype(theano.config.floatX)
        self.U = theano.shared(temp_U, name=tparams_make_name(self.name, 'U'))
        self.U.is_regularizable = True
        self.U.is_trainable = True
        self.b = self._declare_mat('b', 4*self.out_dim, is_regularizable = False)
        self.parameters = [self.W, self.U, self.b] + self.Encoder.parameters + self.Decoder.parameters #, self.p)
        return self

    # Shapes: x_ = (4*out_dim,), child_h = (out_dim, n_steps, arc_types), child_c = (out_dim, n_steps), child_exists = (n_steps, arc_types)
    def recursive_unit(self, x_, child_h, child_c, child_exists, U):
        h_tilde = T.sum(child_h, axis=1) / child_exists.sum()  
        preact = T.tensordot(h_tilde.T, U, [[0,1],[0,1]]) + x_
        i = T.nnet.sigmoid(self.slice(preact, 0, self.out_dim) ) # Input gate
        f = T.nnet.sigmoid(self.slice(x_, 1, self.out_dim).dimshuffle('x', 0) + T.tensordot(child_h, self.slice(U, 1, self.out_dim), [[0, 2],[1, 0]]) )  # Forget gate
        z = T.tanh(self.slice(preact, 3, self.out_dim)) # block input
        c = (f.T * child_c).sum(axis=1) / child_exists.sum() + i * z # cell state
        o = T.nnet.sigmoid(self.slice(preact, 2, self.out_dim) ) # output gate
        h = o * T.tanh(c)  # block output
        return h, c
    ...
    def recursive_unit_batch(self, x_, child_h, child_c, child_exists, U):
        ...
        h_tilde = T.sum(child_h, axis=1) / child_exists[:, None]  
        preact = T.dot(h_tilde, U) + x_
        i = T.nnet.sigmoid(self.slice(preact, 0, self.out_dim) ) # Input gate
    ...
    """ This requires W, U and b initializer 
        and AutoEncoder parameters W, W', b, b'
        Compute auto-encoder score and LSTM output_tv simutaneously
    """
    def compute(self, input_tv, mask=None):
        n_steps = input_tv.shape[0]
        if input_tv.ndim == 3:
        ...
        # t: (1,), node_h: (batch_size, sent_len, out_dim), U: (out_dim, 4*out_dim)
        def __step_batch(x_, m_, t, node_h, node_c, p_score, *nonSeqs):
            child_mask = m_[:, :, 0]
            ...
            self.Encoder.compute(dep_type) 
            arc_emb = self.Encoder.output_tv
            child_h = T.concatenate((node_h, arc_emb), axis=2) * child_mask[:, :, None]
            ...
            diff_mask = T.cast(T.eq(valid_mask, convert_mask), dtype=theano.config.floatX)
            valid_mask += diff_mask
            curr_h, curr_c = self.recursive_unit_batch(x_, child_h, child_c, valid_mask, nonSeqs[0])
            node_h = T.set_subtensor(node_h[:, t, :], curr_h)
            node_c = T.set_subtensor(node_c[:, t, :], curr_c)
            self.Decoder.compute(T.nnet.sigmoid(arc_emb * child_mask[:, :, None])) 
            output = T.nnet.sigmoid(self.Decoder.output_tv)
            transform = T.eye(self.Decoder.out_dim)
            L = - T.sum(transform[dep_type.flatten()].reshape((dep_type.shape[0], dep_type.shape[1], self.Decoder.out_dim)) * T.log(output) 
                + (1.0 - transform[dep_type.flatten()].reshape((dep_type.shape[0], dep_type.shape[1], self.Decoder.out_dim))) * T.log(1-output), axis=2)
            L = T.sum(L * child_mask, axis=1)
            #self.score += T.mean(L)
            return node_h, node_c, T.mean(L)

        x_in = T.dot(input_tv, self.W).astype(config.floatX) + self.b
        seq_in = [x_in, mask, T.arange(n_steps)]
        print 'input for arcTypeEncode_GraphLSTM_layer:', seq_in
        if input_tv.ndim == 3:
            lstm_step = __step_batch
            h_init = T.alloc(np_floatX(0.), n_samples, n_steps, self.out_dim)
            c_init = T.alloc(np_floatX(0.), n_samples, n_steps, self.out_dim)
            ...
            c_init = T.alloc(np_floatX(0.), self.out_dim, n_steps)
        print 'lstm step:', lstm_step
        dummy = theano.shared(np.cast['float32'](0))
        rval, _ = theano.scan(lstm_step,
                              sequences=seq_in,
                              outputs_info=[h_init, c_init, dummy],
                              non_sequences=[self.U] + self.Encoder.parameters + self.Decoder.parameters, #, self.p],
                              go_backwards=self.go_backwards,
                              name=name_tv(self.name, 'arcTypeEncode_GraphLSTM_layer'),
                              n_steps=n_steps,
                              strict=True,
        )
        assert rval[-1].ndim == 1
        self.score = T.mean(rval[-1])
        if input_tv.ndim == 3:
            self.output_tv = rval[0][-1].dimshuffle(1,0,2)
            #self.output_tv = reverse(rval[0][-1].dimshuffle(1,0,2)) if self.go_backwards else rval[0][-1].dimshuffle(1,0,2)
        else:
...
class GraphLSTM_WtdEmbMult(Chip):
    def prepend(self, previous_chip):
        ...
        self.go_backwards = self.params[self.kn('go_backwards')]
        self.W = self._declare_mat('W', self.in_dim, 4*self.out_dim)
        arc_type_dims = self.params[self.kn('type_dim')]
        temp_U = np.zeros((arc_type_dims, self.out_dim, 4*self.out_dim)).astype(theano.config.floatX)
        for i in range(arc_type_dims):
        ...
        self.T = self._declare_mat('T', self.params[self.kn('arc_types')], arc_type_dims)
    ...
    # Shapes: x_= (batch_size, 4*out_dim,), child_h = (batch_size, sent_len, out_dim, type_emb_dim), 
    # child_c = (batch_size, sent_len, out_dim), child_exists = (batch_size, sent_len)
    def recursive_unit_batch(self, x_, child_h, child_c, child_exists, U):
        # Result shape = (batch_size, out_dim, arc_types)
        h_tilde = T.sum(child_h, axis=1) / child_exists[:, None, None] 
        preact = T.tensordot(h_tilde, U, [[1,2],[1,0]]) + x_
        i = T.nnet.sigmoid(self.slice(preact, 0, self.out_dim) ) #+ self.slice(self.p, 0, self.out_dim)*c_prev) # Input gate
        f = T.nnet.sigmoid(self.slice(x_, 1, self.out_dim)[:, None, :] + T.tensordot(child_h, self.slice(U, 1, self.out_dim), [[2,3],[1,0]]) ) #/ child_exists[:, :, None] 
        z = T.tanh(self.slice(preact, 3, self.out_dim)) # block input
        c = (f * child_c).sum(axis=1) / child_exists[:, None] + i * z # cell state
        o = T.nnet.sigmoid(self.slice(preact, 2, self.out_dim) )#+ self.slice(self.p, 2, self.out_dim) * c) # output gate
        h = o * T.tanh(c)  # block output
    ...
    def compute(self, input_tv, mask=None):
        ...
        def __step_batch(x_, m_, t, node_h, node_c, U, type_matrix):
            child_mask = m_[:, :, 0]
            dep_type = T.cast(m_[:, :, 1], dtype='int32')
            # get type embeddings for each arc
            arc_types = type_matrix[dep_type.flatten()].reshape((node_h.shape[0], node_h.shape[1], self.params[self.kn('type_dim')]))
            # compute the embedded hidden vectors
...
class BiLSTMwithAtt(BiLSTM):
    def __init__(self, name, params=None):
        super(BiLSTMwithAtt, self).__init__(name, params)
        print 'print bilstm parameters:', self.params
        self.params[self.name+"_forward_go_backwards"] = False
        self.params[self.name+"_backward_go_backwards"] = True
        self.params[self.name+"_forward_out_dim"] = params[self.kn('out_dim')]
        self.params[self.name+"_backward_out_dim"] = params[self.kn('out_dim')]
        self.params[self.name+"_forward_win_size"] = params[self.kn('win_size')]
        self.params[self.name+"_backward_win_size"] = params[self.kn('win_size')]
        self.forward_chip = LSTMwithAtt(self.name+"_forward", self.params) #
        self.backward_chip = LSTMwithAtt(self.name+"_backward", self.params) #
        #self.params[self.kn('win')] = 2


class BiGraphLSTM_ArcPred(BiLSTM):
    def __init__(self, name, params=None):
        super(BiGraphLSTM_ArcPred, self).__init__(name, params)
        print 'print bi-weighted-graph-lstm parameters:', self.params
        self.params[self.name+"_forward_go_backwards"] = False
        self.params[self.name+"_backward_go_backwards"] = True
        self.params[self.name+"_forward_out_dim"] = params[self.kn('out_dim')]
        ...
        self.params[self.name+"_forward_type_dim"] = params[self.kn('type_dim')]
        self.params[self.name+"_backward_type_dim"] = params[self.kn('type_dim')]
        self.forward_chip = GraphLSTM_ArcPred(self.name+"_forward", self.params) 
        self.backward_chip = GraphLSTM_ArcPred(self.name+"_backward", self.params) 
        #self.params[self.kn('win')] = 2
...
class L2Reg(Chip):
    ...
    def compute(self, input_tv):
        print 'regularizable parameters:', [k for k in self.regularizable_variables()] 
